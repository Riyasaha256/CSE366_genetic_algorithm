...
class Environment:
    ...
    def draw_grid(self, screen, font, class_assignments):
        ...
        color_map = [(0, 0, 255 - i * 50) for i in range(3)]  # 2 color variations based on duration
        ...
        for row in range(self.num_students):
            ...
            availability_text = font.render(f"Availability: {np.sum(self.student_availabilities[row])}", True, (0, 0, 0))
            ...
            for col in range(self.num_classes):
                ...
                if self.student_availabilities[row, assigned_student] == 0:
                    ...
                    pygame.draw.rect(screen, (255, 0, 0), conflict_rect, 3)  # Red border for conflicts
